...
namespace WEB_253503_Kudosh.UI.Services.TelescopeProductService
{
    public class MemoryTelescopeService : ITelescopeService
    {
        ...
        public MemoryTelescopeService([FromServices] IConfiguration config, ICategoryService categoryService)
        {
            _config = config;
        ...
        public Task<ResponseData<ListModel<TelescopeEntity>>> GetProductListAsync(string? categoryNormalizedName, int pageNo = 1)
        {
            ...
            var filteredTelescopes = string.IsNullOrEmpty(categoryNormalizedName) ? 
                _telescopes : _telescopes.Where(t => t.Category.NormalizedName.Equals(categoryNormalizedName, StringComparison.OrdinalIgnoreCase)).ToList();
